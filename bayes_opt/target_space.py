...
class TargetSpace(object):
    ...
    def __init__(self, target_func, pbounds, constraint=None, random_state=None,
                 allow_duplicate_points=False, rate=100, SLO=2000):
        """
        Parameters
        ----------
    ...
    def register(self, params, target, constraint_value=None):
        ...
        self._cost   = np.concatenate([self._cost, [cost]])    # add cost to target space
        # log('data in register: [x--{}, t--{}, cost--{}]'.format(x.reshape(1, -1), target, cost))
        # log('self cost is:', self._cost)
    ...
    def probe(self, params):
        ...
        # 在这里判断是否满足延迟条件，符合才加入 target space
        bs = x[0]
        con = x[1]
        t_require = self.SLO - (bs * con) / self.rate
        if target > t_require:
            return target
    ...
    def cost_min(self):
        """Get minimize cost value found and corresponding parameters.
        
        If there is a constraint present, the minimize value that fulfills the
        constraint is returned."""
        if self._constraint is None:
            try:
                index = self.cost.argmin()
                res = {
                    'cost': self.cost.min(),
                    'target': self.target[index],
                    'params': dict(
                        zip(self.keys, self.params[index])
                    )
                }
            except ValueError:
                res = {'cost': None}
    ...
    def res(self):
        ...
        if self._constraint is None:
            ...
            res = [
                {"target": target, "params": param, "cost": cost}
                for target, param, cost in zip(self.target, params, self.cost)
            ]
            return res
        else:
            # 带 constraint 部分的代码还没有改
