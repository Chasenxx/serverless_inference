...
class BayesianOptimization(Observable):
    ...
    def suggest(self, utility_function):
        ...
        suggestion = acq_max(ac=utility_function.utility,
                             ...
                             y_max=self._space.cost.min(),        # 改好了——需要改成当前最小 cost 对应的 params 
    ...
    def cost_efficient(self,
        ...
        if acquisition_function is None:
            util = UtilityFunction(kind='cost_ei',
                                   kappa=2.576,
                                   xi=0.0,
                                   kappa_decay=1,
                                   kappa_decay_delay=0,
        ...
        while not self._queue.empty or iteration < n_iter:
            try:
                ...
                util.update_params()
                x_probe = self.suggest(util)    # 确定下一个采样点
                iteration += 1
            self.probe(x_probe, lazy=False)     # 在 target space 中进行采样
