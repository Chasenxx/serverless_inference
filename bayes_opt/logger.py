...
class ScreenLogger(_Tracker):
    ...
    def _step(self, instance, colour=Colours.black):
        ...
        # 添加 cost 列
        cells.append(self._format_number(res["cost"]))
    ...
    def _header(self, instance):
        ...
        cells.append(self._format_key("cost"))
    ...
    # 判断新的最低 cost 值  -------------------- 需要结合 previous 修改
    def _is_new_cost(self, instance):
        if instance.cost_min["cost"] is None:
            return False
        if self._previous_cost_min is None:
            self._previous_cost_min = instance.cost_min["cost"]
        return instance.cost_min["cost"] < self._previous_cost_min

    def update(self, event, instance):
        if event == Events.OPTIMIZATION_START:
            line = self._header(instance) + "\n"
        elif event == Events.OPTIMIZATION_STEP:
            is_new_cost = self._is_new_cost(instance)
            if self._verbose == 1 and not is_new_cost:
                line = ""
            else:
                colour = Colours.purple if is_new_cost else Colours.black
